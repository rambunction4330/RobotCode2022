...
bool checkREVLibError(rev::REVLibError error, const char* functionCall, const char* fileName, int line) {
  ...
  wpi::outs() << "[ SparkMax ERROR " << REVLibErrorStrings[(int)error] << " ] in " 
    << fileName << ":" << std::to_string(line) << " \"" << functionCall << "\"\n"; 
