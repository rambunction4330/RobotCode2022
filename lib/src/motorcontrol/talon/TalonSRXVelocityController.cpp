...
namespace rmb {
  template<typename U>
  TalonSRXVelocityController<U>::TalonSRXVelocityController(int deviceNumber) : talonSRX(deviceNumber) {
    talonSRX.ConfigFactoryDefault();
    talonSRX.ConfigSelectedFeedbackSensor(ctre::phoenix::motorcontrol::FeedbackDevice::CTRE_MagEncoder_Relative, 0);
  }
  ...
  TalonSRXVelocityController<U>::TalonSRXVelocityController(int deviceNumber, 
    ...
    talonSRX.ConfigAllowableClosedloopError(0, RawVelocity_t(config.allowableError / conversion).to<double>());
    talonSRX.ConfigClosedLoopPeakOutput(config.maxOutput);
    talonSRX.ConfigSelectedFeedbackSensor(ctre::phoenix::motorcontrol::FeedbackDevice::CTRE_MagEncoder_Relative, 0);
  }
  ...
  void TalonSRXVelocityController<U>::setVelocity(Velocity_t velocity) {
    ...
    talonSRX.Set(ctre::phoenix::motorcontrol::TalonSRXControlMode::Velocity, setPoint);
  }

  template<typename U>
  typename TalonSRXVelocityController<U>::Velocity_t TalonSRXVelocityController<U>::getVelocity() {
