...
namespace rmb {
  template<typename U>
  TalonFXVelocityController<U>::TalonFXVelocityController(int deviceNumber) : talonFX(deviceNumber) {
  ...
  void TalonFXVelocityController<U>::setVelocity(Velocity_t velocity) {
    ...
    talonFX.Set(ctre::phoenix::motorcontrol::TalonFXControlMode::Velocity, setPoint);
  }
