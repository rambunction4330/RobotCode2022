...
namespace rmb {
  ...
  class FeedForward {
    public:
      ...
      // using KvUnit = units::compound_unit<units::volts, units::inverse<Velocity>>;
      // using Kv_t = units::unit_t<KvUnit>;
      // using KaUnit = units::compound_unit<units::volts, units::inverse<Acceleration>>;
      // using Ka_t = units::unit_t<KaUnit>;

      virtual units::volt_t calculate(Velocity_t velocity, Distance_t distance, Acceleration_t acceleration) = 0;
  };
}