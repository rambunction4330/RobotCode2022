...
public:
  ...
  ElevatorFeedforward(Ks_t kS, Ks_t kG, Kv_t kV, Ka_t kA) : kS(kS), kG(kG), kV(kV), kA(kA) {}

  inline units::volt_t
  calculate(Velocity_t velocity, Distance_t distance = Distance_t(0.0), Acceleration_t acceleration = Acceleration_t(0.0)) const override {
    return kS * wpi::sgn(velocity) + kG + kV * velocity + kA * acceleration;
  }

  inline Velocity_t maxAchievableVelocity(units::volt_t maxVoltage, Acceleration_t acceleration, Distance_t position = Distance_t(0.0)) const override {
    return (maxVoltage - kS - kG - kA * acceleration) / kV;
  }

  inline Velocity_t minAchievableVelocity(units::volt_t maxVoltage, Acceleration_t acceleration, Distance_t position = Distance_t(0.0)) const override {
    return (-maxVoltage + kS - kG - kA * acceleration) / kV;
  }

  inline Acceleration_t maxAchievableAcceleration(units::volt_t maxVoltage, Velocity_t velocity, Distance_t position = Distance_t(0.0)) const override {
    return (maxVoltage - kS * wpi::sgn(velocity) - kG - kV * velocity) / kA;
  }

  inline Acceleration_t minAchievableAcceleration(units::volt_t maxVoltage, Velocity_t velocity, Distance_t position = Distance_t(0.0)) const override {
  ...
  inline units::volt_t calculateStatic(Velocity_t velocity, Distance_t position = Distance_t(0)) const override {
