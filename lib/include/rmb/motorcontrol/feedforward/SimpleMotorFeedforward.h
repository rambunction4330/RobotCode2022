...
public:
  ...
  inline units::volt_t calculate(Velocity_t velocity, 
  ...
  inline Acceleration_t maxAchievableAcceleration(units::volt_t maxVoltage, 
                                                  Velocity_t velocity, 
                                                  Distance_t position = Distance_t(0.0)) const override{
    return (maxVoltage - kS * wpi::sgn(velocity) - kV * velocity) / kA;
  }

  inline Acceleration_t minAchievableAcceleration(units::volt_t maxVoltage, 
                                                  Velocity_t velocity, 
                                                  Distance_t position = Distance_t(0.0)) const override {
...
template <typename U>
SimpleMotorFeedforward<U> noFeedforward(typename SimpleMotorFeedforward<U>::Ks_t(0.0),
